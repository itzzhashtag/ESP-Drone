// ================================================================
//  Q450 ESP32 Flight Controller  —  v1.4
...
// ----------------------------------------------------------------
//  CHANGES v1.3 → v1.4
//  • PITCH_FLIP set to -1.0f — IMU mounted with Y axis inverted
//    Nose-down now correctly reads as negative pitch
//  • IMU calibration offsets added (CALIBRATE_IMU routine used)
//    IMU_OFF_AX/AY/AZ/GX/GY/GZ filled with measured values
//  • captureGroundReference() now uses fast accelerometer method
//    (~0.75s at boot) instead of waiting 30s for filter convergence
//  • Outlier rejection added to ground reference capture
//    Samples differing >2° from rough average are discarded
//  • Capacitor recommendations added to initIMU() hardware note
//    100nF ceramic + 10µF electrolytic on MPU VCC/GND
//    100nF ceramic + 10µF electrolytic on ESP32 3.3V/GND
//
// ----------------------------------------------------------------
...
#define THROTTLE_MAX   1800    // µs  ★ TUNE ME — raise slowly in flight
...
#define THROTTLE_RAMP_RATE   2    // µs/tick  ★ TUNE ME
...
#define MAX_MOTOR_STEP   30    // µs/tick  ★ TUNE ME
...
//
//              Roll     Pitch    Yaw
float KP[3] = { 1.50f,   1.50f,   0.80f };  // ★ START HERE
